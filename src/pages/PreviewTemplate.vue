			...
			<div class="header__container _container">
				<div class="header__top">
					...
					<MenuItems :style="{ 'background': `linear-gradient(0deg, rgba(255, 247, 235, 0.9), rgba(255, 247, 235, 0.9)), url(${require('@/assets/bg-templates/bg-template-1.jpg')}) repeat 10%` }" class="menu__body _container">
						...
						:key="category.value"
						class="menu__item">
									<button class="menu__link category__title _icon-arrow-down" data-spoller
										type="button">{{ category.label}}</button>
									<ul class="menu__sub-list">
										<li 
										v-for="rubric in category.rubrics"
										:key="rubric.value"
										class="menu__sub-item">
											<a href="#rup__title_cold-snacks"
												class="menu__sub-link rup__title _goto-block">{{ rubric.label }}</a>
										</li>
	...
	...mapState({
      ...
      langListReserve: state => state.user.langListReserve, 
		...
		swiper() {
			...
			menuWatch() {
      return this.$vnode.data?.class?.["_active"];
    },
  },
  watch(){
	// unlock(){
